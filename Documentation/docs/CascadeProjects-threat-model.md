...
7. A compromised local client uses broad observability tools (`seeds-server`, `pulse-server`, `maintain-server`) to enumerate repos, process state, journals, workflow history, and journaling metadata. Impact: confidentiality loss about code, operations, and current work.

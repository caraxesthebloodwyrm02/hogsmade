...
| `Tools/MCPServers/` | First-party MCP servers | TS (`afloat-server`, `echoes-server`, `eligibility-server`, `grid-server`, `lots-server`, `maintain-server`, `ori-server`, `overview-server`, `pulse-server`, `seeds-server`) plus watch-period `harness-server`, `nexus-server`, `school-server`; archived `craft-server`, `glimpse-server`, `mangrove-server` (see `mcp_inventory.manifest.json` for the authoritative list) |

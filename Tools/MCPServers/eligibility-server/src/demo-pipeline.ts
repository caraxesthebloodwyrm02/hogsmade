...
function buildTopologyArtifact(report: {
  ...
  return {
    ...
    synopsis:
      "A debugger-style topology artifact showing how one execution source fans out into routine and cycle surfaces.",
    nodes: [
      {
        id: "scenario",
    ...
    edges: [
      {
        from: "scenario",
        to: "normalize",
        reason: "Scenario args determine the runtime signature.",
      },
      {
        from: "normalize",
        to: "hierarchy",
        reason: "Normalized biases shape weight derivation and ranking.",
      },
      {
        from: "hierarchy",
        to: "forms",
        reason: "Hierarchy drives form compilation and collection rows.",
      },
      {
        from: "forms",
        to: "endpoint",
        reason: "Runtime artifacts inform the endpoint readiness update.",
      },
      {
        from: "endpoint",
        to: "promotion",
        reason: "Endpoint readiness and signals flow into the promotion gate.",
      },
      {
        from: "promotion",
        to: "support-balance-assist",
        reason:
          "Assist summarizes support readiness and cross-dimension balance after gate evaluation.",
      },
    ],
  };
}

function buildSupportBalanceAssist(
  routine: RoutineResult,
  cycle: ExecuteReport["cycleSnapshots"],
  ...
  const readSlice = (dimension: HierarchySlice["dimension"]): number =>
    routine.hierarchy.find(
      (slice) => slice.candidateId === leaderId && slice.dimension === dimension,
    )?.score ?? 0;
  ...
  const spread =
    Math.max(governance, usability, integration, observability) -
    Math.min(governance, usability, integration, observability);
...
function buildTopologyStory(artifact: TopologyArtifact): string {
  return [
    ...
    artifact.synopsis,
    ...artifact.nodes.map(
      (node) => `- ${node.tier.toUpperCase()} :: ${node.label} -> ${node.highlight}`,
    ),
...
export class EligibilityRouter {
  async execute(
    ...
    const signalSnapshots = scenario.cycle.signals
      .map((signal) =>
        recordCycleSignal(
          {
            caseId: scenario.cycle.caseId,

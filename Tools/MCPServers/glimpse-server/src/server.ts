...
// Avoid deep generic instantiation in strict TS builds.
...
tool(
  ...
  "Run the full Glimpse enhanced pipeline on a dataset. Returns context lenses, relations, confidence report, invariant patterns, complexity assessment, and grounded insights.",
  {
    data: z
      ...
      .describe("Array of data records (objects with string keys)"),
    fileType: z.enum(["json", "csv"]).default("json").describe("Data format"),
    config: z
      .record(z.string(), z.any())
      .optional()
      .describe("Master config override. If omitted, uses minimal defaults."),
  ...
  async ({ data, fileType, config: userConfig, multiPass, grounding }: any) => {
    ...
    try {
      ...
      if (!result) {
        return {
          content: [
            { type: "text" as const, text: "Pipeline returned null — empty or invalid data." },
          ],
        };
      }
      ...
      const summary = {
        ...
        contextLenses: result.contextLenses?.map((l: any) => ({
          label: l.label,
          score: l.score,
          role: l.role,
        })),
...
tool(
  ...
  async ({ entities, relationCount, descriptorCount }: any) => {
...
tool(
  ...
  async ({ insightText, evidences, lenses }: any) => {
...
tool(
  ...
  async ({ entities, relations, evidences }: any) => {
...
tool(
  ...
  "Run a full Glimpse pipeline session on GRID event data. Returns complete analysis including context lenses, confidence report, invariant patterns, grounded insights, and complexity assessment.",
  {
    records: z
      .array(z.record(z.string(), z.any()))
      .describe("Array of event records (from GRID /api/v1/glimpse/feed or inline)"),
  ...
  async ({ records, config: userConfig, options }: any) => {
...
tool(
  ...
  async ({ events, sessionId }: any) => {
    ...
    try {
      ...
      if (!result) {
        return {
          content: [
            {
              type: "text" as const,
              text: "Pipeline returned null — insufficient accumulated data.",
            },
          ],
        };
      }
      ...
      try {
        refinement = eng.learnFromRun(session.records, result, session.config, {
          source: sessionId,
          elapsed: 0,
        });
      } catch (_learnErr: any) {
...
tool(
  ...
  "Evaluate PATH system (condition-driven weighted accumulation) on session data. Returns scored paths with signal inventory.",
  {
    ...
    pathIds: z
      .array(z.string())
      .optional()
      .describe("Specific path IDs to evaluate (default: all builtin)"),
  },
  async ({ data, pathIds }: any) => {

...
function createStore() {
  return new EvolutionCycleStore(
    path.join(mkdtempSync(path.join(tmpdir(), "eligibility-root-cycle-")), "cases.json"),
  );
}
...
test("built package evaluates a promotion gate after endpoint and signal accumulation", () => {
  ...
  assert.ok(
    ["allow_promotion", "hold_for_tighten", "return_to_balance"].includes(evaluated.gate.decision),
  );

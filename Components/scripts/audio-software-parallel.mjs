...
function distortion(sig, { drive = 0, tone = 0.5 } = {}) {
  ...
  if (drive > 0) {
    ...
    return {
      ...
      _meta: {
        name: "🎸 Distortion",
        drive: +effectiveDrive.toFixed(2),
        tone,
        mod: +mod.toFixed(3),
        active: true,
      },
  ...
  return {
    ...sig,
    _meta: { name: "🎸 Distortion [BYPASS]", drive, tone, lfoFreq: "2Hz", active: false },
  };
}
...
function delay(sig, { time = 0, feedback = 0, mix = 0 } = {}) {
  ...
  return {
    ...sig,
    _meta: { name: "🎸 Delay [BYPASS]", time, feedback, mix, lfoFreq: "0.3Hz", active: false },
  };
}
...
function wah(sig, { position = 0.5, resonance = 0 } = {}) {
  ...
  return {
    ...sig,
    _meta: { name: "🎸 Wah [BYPASS]", position, resonance, lfoFreq: "1.5Hz", active: false },
  };
}
...
function zodValidation(req) {
  ...
  return {
    ...
    _meta: {
      name: "Zod Validation (≈ Gain Staging)",
...
function pipeline(req) {
  ...
  return {
    ...
    _meta: {
      name: "Pipeline (≈ Signal Chain)",
      stages,
      perStageCost: +perStageCost.toFixed(1),
      audio: "Signal Chain",
    },
  };
}
...
function healthFeedback(req) {
  ...
  return {
    ...
    coverage: req.coverage + 0.1,
    _meta: {
      name: "Health Feedback (≈ Feedback Loop)",
      adjustment: +adjustment.toFixed(2),
      audio: "Reverb/Sidechain",
    },
  };
}
...
function rateLimiter(req, { burstSize = 10, tokensPerSec = 5 } = {}) {
  ...
  return {
    ...
    coverage: req.coverage + 0.1,
    _meta: {
      name: "Rate Limiter (≈ Master Compressor)",
      burstSize,
      throttled: +throttled.toFixed(3),
      audio: "Compressor",
    },
  };
}
...
function chaosInjection(req) {
  return {
    ...req,
    _meta: { name: "🎸 Chaos Injection [PLACEHOLDER]", active: false, audio: "Distortion" },
  };
}
...
function adaptiveRouting(req) {
  return {
    ...req,
    _meta: { name: "🎸 Adaptive Routing [PLACEHOLDER]", active: false, audio: "Wah" },
  };
}
...
function cmdAnalyze() {
  if (!existsSync(RUNS_FILE)) {
    console.error("No runs yet. Run: node scripts/audio-software-parallel.mjs run");
    process.exit(1);
  }
  ...
  const parallel = [
    {
      audio: "Peak Reduction (dB)",
      software: "Latency Added (ms)",
      audioVals: peakReductions,
      softVals: latencyAdded,
    },
    {
      audio: "RMS Change (dB)",
      software: "Throughput Change",
      audioVals: rmsChanges,
      softVals: throughputChanges,
    },
  ];
  ...
  for (const p of parallel) {
    ...
    console.log(
      `  │  Audio   avg=${avg(p.audioVals).toFixed(2)}  σ=${stddev(p.audioVals).toFixed(3)}`,
    );
    console.log(
      `  │  Soft    avg=${avg(p.softVals).toFixed(2)}  σ=${stddev(p.softVals).toFixed(3)}`,
    );

    // Correlation coefficient
    const n = p.audioVals.length;
    const aAvg = avg(p.audioVals),
      sAvg = avg(p.softVals);
    let num = 0,
      denA = 0,
      denS = 0;
    for (let i = 0; i < n; i++) {
      const da = p.audioVals[i] - aAvg,
        ds = p.softVals[i] - sAvg;
      num += da * ds;
      denA += da * da;
      denS += ds * ds;
    }
...
function cmdHtml() {
  ...
  const softStageData = softStages.map((name, i) => {
    ...
    return {
      name,
      latency: avgOf("latency"),
      throughput: avgOf("throughput"),
      health: avgOf("health"),
      errors: avgOf("errors"),
    };
  });
  ...
  const parallelMap = [
    {
      audio: "Compressor",
      software: "Zod Validation",
      concept: "Gain Staging → Input Normalization",
    },
    ...
    {
      audio: "Master Compressor",
      software: "Rate Limiter",
      concept: "Dynamic Range → Traffic Shaping",
    },
    {
      audio: "Transient Shaper",
      software: "Circuit Breaker",
      concept: "Attack Control → Failure Threshold",
    },
    {
      audio: "Dynamics Shaper",
      software: "Data Contracts",
      concept: "Expansion/Gate → Interface Contracts",
    },
    { audio: "Limiter", software: "Aggregation", concept: "Hard Ceiling → Output Mixing" },
    {
      audio: "🎸 Distortion",
      software: "🎸 Chaos Injection",
      concept: "Harmonic Saturation → Fault Injection",
    },
    { audio: "🎸 Delay", software: "🎸 Retry Queue", concept: "Time Repeat → Retry with Backoff" },
    {
      audio: "🎸 Wah",
      software: "🎸 Adaptive Routing",
      concept: "Frequency Sweep → Dynamic Routing",
    },
  ];

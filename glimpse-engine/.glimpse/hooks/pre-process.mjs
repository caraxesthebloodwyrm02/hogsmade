...
// Save error output
writeFileSync(path.join(TEMP_DIR, `${name.replace(/\s+/g, '-')}.err`), 
...
async function cleanup() {
  ...
  log('info', `${C.cyan}Process completed: ${stages.filter(s => s.status === 'success').length}/${stages.length} stages, ${errors} errors, ${duration}ms total${C.reset}`);
}
...
╚══════════════════════════════════════════════════════════════════╝${C.reset}\n`);
  ...
  try {
    ...
    await runStage('structure check', 'find core functions analysis -name "*.js" | head -20', false);
    ...
    await runStage('organize', 'node -e "import(\'./core/validators/index.js\').then(v => console.log(\'✓ Validators exposed\')).catch(e => console.error(\'✗\', e.message))"', false);
    
    // STAGE 5: Compile/Bootstrap
    await runStage('compile/bootstrap', 'node --test tests/validators/sync-validator.test.js --test tests/validators/calibration-engine.test.js 2>&1 | tail -5', false);
    ...
    writeFileSync(path.join(ROOT, '.glimpse', 'last-run.json'), JSON.stringify({
      runId: RUN_ID,

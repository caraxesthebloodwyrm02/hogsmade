    ...
    if (!contractReport.valid) {
      ...
      if (!json) {
        ...
        if (contractReport.missing.length) {
          console.error(`      Missing implementations: ${contractReport.missing.slice(0, 5).join(', ')}${contractReport.missing.length > 5 ? '...' : ''}`);
        }
        if (contractReport.orphaned.length) {
          console.error(`      Orphaned implementations: ${contractReport.orphaned.slice(0, 5).join(', ')}${contractReport.orphaned.length > 5 ? '...' : ''}`);
        }
      ...
      if (!json) {
        console.log(`   ✅ Function contracts valid (${contractReport.summary.totalDeclared} functions)`);
        if (verbose) {

...
import {
  runContextPipeline,
  runContextPipelineAsync,
  compileRuleFromConversation,
  parseQueryIntent,
  validateConfigWithRegistry,
} from "../glimpse-engine/core/engine.js";
...
test("async pipeline supports web grounding without null confidence values", async () => {
  const data = await loadJson("sample-innovations.json");
  const ctx = await runContextPipelineAsync(data, "json", config, {
    presetId: "researcher",
    grounding: true,
    groundingMode: "web",
    searchFn: async () => [{ title: "reference" }],
  });
...
test("mixed ambiguous data remains stable across repeated runs", () => {
  ...
  const first = runContextPipeline(data, "json", config, {
    presetId: "analyst",
  });
  const second = runContextPipeline(data, "json", config, {
    presetId: "analyst",
  });
...
test("disabling a rule removes its effect without breaking the rest of the pipeline", async () => {
  ...
  const ctx = runContextPipeline(data, "json", withoutGeoRule, {
    presetId: "storyteller",
  });
...
test("invalid function names fail closed and appear in diagnostics", () => {
  ...
  const ctx = runContextPipeline([{ token: "alpha" }], "json", broken, {
    presetId: "analyst",
  });

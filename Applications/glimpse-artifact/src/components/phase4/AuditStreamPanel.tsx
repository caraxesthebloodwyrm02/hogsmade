...
function Sparkline({ events }: { events: AuditEvent[] }) {
  ...
  const points = buckets.map((v, i) => `${i * step},${h - (v / max) * (h - 4)}`).join(" ");
  ...
  return (
    <svg width={w} height={h} className="shrink-0" aria-hidden="true">
      ...
      />
      {buckets.map((v, i) => (
        <circle key={i} cx={i * step} cy={h - (v / max) * (h - 4)} r={1.5} fill="var(--teal-500)" />
      ))}
    </svg>
  );
}
...
export function AuditStreamPanel({
  ...
  onPauseToggle,
  isPaused,
}: AuditStreamPanelProps) {
  const [activeFilters, setActiveFilters] = useState<Set<string>>(new Set());
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  // A2: Dynamic server filters from actual event sources
  const allSources = useMemo(() => [...new Set(events.map((e) => e.source))].sort(), [events]);
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-ink truncate flex-1">{evt.tool}</span>
                    <span
                      className={cn(

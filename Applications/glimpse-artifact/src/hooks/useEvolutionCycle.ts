...
export function useEvolutionCycle(): UseEvolutionCycleResult {
  ...
  const updateFromMutation = useCallback(
    async (response: Response) => {
      const payload = await readJson<{ snapshot?: CycleSnapshot; gate?: PromotionGateResult }>(
        response,
      );
      if (payload.snapshot) {
        setSnapshot(payload.snapshot);
  ...
  const openCase = useCallback(
    async (input: EvolutionOpenInput) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const recordSignal = useCallback(
    async (caseId: string, type: CycleSignal["type"], note?: string) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const recordHandoffAction = useCallback(
    async (caseId: string, input: EvolutionHandoffInput) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const upsertEndpointAction = useCallback(
    async (caseId: string, input: EvolutionEndpointInput) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const advanceCase = useCallback(
    async (caseId: string, direction: "forward" | "return" = "forward", reason?: string) => {
      setLoading(true);
      setError(null);
      try {

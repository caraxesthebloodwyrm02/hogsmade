...
function scoreViewBase(id, context, preset, config) {
  ...
  if (id === "map")
    support = profile.flags.has_space_dimension
      ? profile.flags.has_geo_coordinates
        ? 0.95
        : 0.8
      : 0;
  if (id === "explorer") support = 0.95;
  if (id === "matrix")
    support = profile.flags.has_metric_dimension || context.relations.length ? 0.85 : 0.35;
  if (id === "flow")
    support = context.relations.some((relation) => relation.type === "influenced") ? 0.92 : 0.2;
...
function renderConstellation(context, state) {
  ...
  const nodes = context.entities
    .filter((entity) => nodeIds.has(entity.id))
    .map((entity) => ({
      id: entity.id,
      name: entity.name,
      score:
        Object.keys(entity.metrics).length +
  ...
  const svg = [];
  svg.push(
    `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Constellation view of ${nodes.length} entities and ${edges.length} relations">`,
  );
  svg.push(
    '<defs><marker id="arrowhead" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="var(--teal-500)" opacity="0.7"/></marker></defs>',
  );
  edges.forEach((edge) => {
    const source = nodeMap.get(edge.source);
    const target = nodeMap.get(edge.target);
    if (!source || !target) return;
    const cls =
      edge.type === "influenced"
        ? "edge-line edge-influenced"
        : edge.type === "shared-space"
          ? "edge-line edge-shared-space"
          : edge.type === "shared-domain"
            ? "edge-line edge-shared-domain"
            : "edge-line edge-shared-era";
    if (edge.type === "influenced") {
      ...
      svg.push(
        `<line class="${cls}" x1="${source.x}" y1="${source.y}" x2="${target.x}" y2="${target.y}"/>`,
      );
    }
  });
  nodes.forEach((node) => {
...
function renderTimeline(context) {
  const timeEntities = context.entities.filter(
    (entity) => typeof entity.dimensions.time === "number",
  );
  ...
  const scale = (year) =>
    pad.l + ((year - minYear) / (maxYear - minYear)) * (width - pad.l - pad.r);

  const svg = [
    `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Timeline view of ${timeEntities.length} entities">`,
    '<defs><marker id="timelineArrow" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto"><polygon points="0 0, 7 2.5, 0 5" fill="var(--teal-500)" opacity="0.7"/></marker></defs>',
  ];
  ...
  context.relations
    .filter((relation) => relation.type === "influenced")
    .forEach((relation) => {
      ...
      if (!source || !target) return;
      const sourceLane = laneNames.indexOf(
        source.dimensions.domain || context.primaryLens?.label || source.type,
      );
      const targetLane = laneNames.indexOf(
        target.dimensions.domain || context.primaryLens?.label || target.type,
      );
      ...
      svg.push(
        `<line class="timeline-connector" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" marker-end="url(#timelineArrow)"/>`,
      );
    });
...
function renderMatrix(context) {
  ...
  const pairScore = (left, right) => {
    ...
    if (
      left.dimensions.space &&
      right.dimensions.space &&
      String(left.dimensions.space).toLowerCase() === String(right.dimensions.space).toLowerCase()
    )
      score += 0.25;
    if (
      left.dimensions.domain &&
      right.dimensions.domain &&
      String(left.dimensions.domain).toLowerCase() === String(right.dimensions.domain).toLowerCase()
    )
      score += 0.3;
...
function renderFlow(context) {
  ...
  const roots = unique(influences.map((relation) => relation.source)).filter(
    (source) => !incoming.has(source),
  );
  const layers = [];
  ...
  while (frontier.length) {
    ...
    frontier = influences
      .filter((relation) => frontier.includes(relation.source))
  ...
  const svg = [
    ...
    '<defs><marker id="flowArrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="var(--teal-500)"/></marker></defs>',
  ];
  const positions = {};
...
function renderMap(context) {
  ...
  const svg = [
    `<svg viewBox="0 0 ${width} ${height}">`,
    '<rect x="0" y="0" width="900" height="460" rx="20" fill="url(#mapBg)"/>',
    '<defs><linearGradient id="mapBg" x1="0" x2="1"><stop offset="0%" stop-color="#ecfeff"/><stop offset="100%" stop-color="#f8fafc"/></linearGradient></defs>',
  ];
  ...
  entries.forEach(([place, members], index) => {
    ...
    svg.push(
      `<circle cx="${cx}" cy="${cy}" r="${radius}" fill="var(--teal-100)" stroke="var(--teal-500)" stroke-width="2"/>`,
    );

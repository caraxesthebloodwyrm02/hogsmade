...
export const CALIBRATION_POLICIES = {
  strict: {
    ...
    thresholds: {
      LOW_COVERAGE: 0.50,      // Changed from static 0.3
      WEAK_BASIS: 0.65,       // Changed from static 0.5
      MISSING_DIMENSION: 0.50, // Changed from static 0.3
  ...
  const engine = {
    ...
    detectGapsPolicy(frame, ctx) {
      ...
      const lowConfidenceEntities = entities.filter(e => {
        const entityEvidence = evidences?.filter(ev => 
          ev.scope === 'entity' && ev.targetId === e.id
        ) || [];
        const avgConfidence = entityEvidence.length > 0
...
export function comparePolicies(policyA, policyB) {
  ...
  return {
    ...
    recommendation: Object.keys(stricter).length > Object.keys(looser).length

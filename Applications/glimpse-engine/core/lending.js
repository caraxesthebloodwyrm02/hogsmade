...
export function analyzeLending(contextData) {
  ...
  if (history) {
    ...
    factors.push({
      ...LENDING_FACTORS.repayment_history,
      score: reliability,
      detail: `${history.repaid}/${history.pastLends} repaid`,
    });
  }
  ...
  if (finances && request) {
    ...
    const capacityScore =
      monthsLeft >= 6 ? 0.9 : monthsLeft >= 3 ? 0.6 : monthsLeft >= 1 ? 0.3 : 0.1;
    factors.push({
      ...LENDING_FACTORS.financial_capacity,
      score: capacityScore,
      detail: `${monthsLeft.toFixed(1)} months runway after`,
    });
  }
  ...
  if (history) {
    const outstandingScore =
      history.outstanding === 0 ? 0.9 : history.outstanding < 10000 ? 0.5 : 0.2;
    factors.push({
      ...LENDING_FACTORS.outstanding_debt,
      score: outstandingScore,
      detail:
        history.outstanding > 0
          ? `৳${history.outstanding.toLocaleString()} outstanding`
          : "clean slate",
    });
  }
  ...
  if (market) {
    ...
    factors.push({
      ...LENDING_FACTORS.business_viability,
      score: viabilityMap[market.businessViability] || 0.4,
      detail: market.localDemand || "no demand data",
    });
  }
  ...
  if (request) {
    ...
    factors.push({
      ...LENDING_FACTORS.relationship_strength,
      score: relMap[request.relationship] || 0.4,
      detail: request.relationship,
    });
  }
  ...
  if (emotional) {
    ...
    factors.push({
      ...LENDING_FACTORS.emotional_pressure,
      score: pressureMap[emotional.pressure] || 0.5,
      detail: emotional.pressure + " pressure",
    });
  }
  ...
  if (weightedScore >= 0.7) {
    ...
    recommendation = {
      signal: "conditional",
      text: "CONDITIONAL — consider partial amount or structured repayment",
    };
  } else {
  ...
  return {
    ...
    summary: {
      ...
      topStrength:
        factors.filter((f) => f.score >= 0.7).sort((a, b) => b.score - a.score)[0] || null,
    },
  };
}

...
import { clamp, resolvePath, unique, bucketYear } from "../utils/utils.js";
...
export function temporal_distance(context, args) {
  if (context.relation) {
    ...
    return {
      ...
      score: Number.isFinite(gap)
        ? clamp(1 - gap / Math.max(Number(args.max_gap) || 10, 1), 0, 1)
        : 0,
  ...
  if (context.entity) {
    ...
    const closeEntities = entities.filter((other) => {
      ...
      return (
        Number.isFinite(otherTime) && Math.abs(entityTime - otherTime) <= Number(args.max_gap || 10)
      );
    });
  ...
  if (timeValues.length < 2) {
    return {
      ...
      value: 0,
      score: 0,
      reason: "Insufficient temporal data for clustering analysis",
    };
  }
...
export function shared_dimension(context, args) {
  ...
  if (context.relation) {
    ...
    if (left == null || right == null) {
      return {
        ...
        value: null,
        score: 0,
        reason: `${dimension} dimension missing for one or both entities.`,
      };
    }
    ...
    return {
      ...
      score: similarity,
      reason: exactMatch
        ? `${dimension} matches across the relation.`
        : `${dimension} does not match across the relation.`,
  ...
  if (context.entity) {
    ...
    if (entityValue == null) {
      return {
        ...
        value: null,
        score: 0,
        reason: `Entity has no ${dimension} dimension.`,
      };
    }
    ...
    const sharedEntities = entities.filter((other) => {
      ...
      return (
        otherValue != null && String(otherValue).toLowerCase() === String(entityValue).toLowerCase()
      );
    });
  ...
  return {
    ...
    value: null,
    score: 0,
    reason: "Shared dimension analysis requires entity or relation context.",
  };
}
...
export function influence_link(context, args) {
  if (context.relation) {
    ...
    return {
      ...
      reason: isInfluenced
        ? "Influence structure is present in this relation."
        : "No influence structure in this relation.",
    };
  }
...
export function density_score(context, args) {
  ...
  if (entities.length < 2) {
    return {
      ...
      value: 0,
      score: 0,
      reason: "Insufficient entities for density calculation.",
    };
  }

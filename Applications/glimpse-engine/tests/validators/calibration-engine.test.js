...
test("Strict policy has highest thresholds", () => {
  assert.ok(
    CALIBRATION_POLICIES.strict.thresholds.LOW_COVERAGE >=
      CALIBRATION_POLICIES.adaptive.thresholds.LOW_COVERAGE,
  );
  assert.ok(
    CALIBRATION_POLICIES.strict.thresholds.WEAK_BASIS >=
...
test("detectGapsPolicy respects policy thresholds", () => {
  ...
  assert.ok(
    frame1.gaps.length >= frame2.gaps.length,
...
test("suggestAdjustments suggests lowering thresholds when few gaps", () => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const ctx = {
      ...
      evidences: Array.from({ length: 5 }, (_, j) => ({
        id: `e${i}-${j}`,
        confidence: 0.9,
        scope: "entity",
        targetId: "1",

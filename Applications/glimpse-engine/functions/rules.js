...
import { clamp, clone, compareFact, resolvePath, unique } from "../utils/utils.js";
import { createEvaluationContext, createSafeFunctionRegistry } from "./functions.js";
...
function inferRuleScope(rule, config) {
  ...
  const facts = [...(rule.when || []), ...(rule.guards || [])].map(
    (condition) => condition.fact || "",
  );
  if (
    facts.some(
      (fact) =>
        fact.startsWith("relation.") || fact.startsWith("source.") || fact.startsWith("target."),
    )
  )
    return "relation";
...
function evaluateGuardSet(rule, context, scopeType, registry, trace) {
  ...
  for (const guard of guards) {
    ...
    if (!result.passed) {
      ...
      trace.guardFailures.push(
        guard.function ? `Guard ${guard.function} returned false.` : `${guard.fact} did not match.`,
      );
      return { ok: false };
    }
  }
...
function deriveConfidence(rule, result, config) {
  ...
  if (strategy === "direct_score")
    return clamp(Math.max(floor, Number(result.score ?? result.value ?? 0)), floor, 1);
  if (strategy === "normalized_value")
    return clamp(Math.max(floor, Number(result.value || 0)), floor, 1);
...
function applyAction(action, context, evidence, state, trace) {
  if (action.action === "boost_lens") {
    ...
    if (!state.lensBuckets[lens]) {
      state.lensBuckets[lens] = {
        score: 0,
        evidenceIds: [],
        label: state.lensLabels[lens] || lens,
      };
    }
    ...
    if (context.entity?.id) {
      ...
      state.entityLensScores[context.entity.id][lens] =
        (state.entityLensScores[context.entity.id][lens] || 0) + weight * evidence.confidence;
    }
  }
  ...
  if (action.action === "prefer_view") {
    ...
    state.viewPreferences[view] =
      (state.viewPreferences[view] || 0) + Number(action.score || 0) * evidence.confidence;
...
export function applyRules(config, datasetScope, entities, relations) {
  ...
  const state = {
    lensBuckets: {},
    lensLabels: Object.fromEntries(
      (config.taxonomy?.domains || []).map((domain) => [domain.id, domain.label]),
    ),
    entityLensScores: {},
    viewPreferences: {},
    evidences: [],
  ...
  (config.rules || [])
    ...
    .forEach((rule) => {
      ...
      const items =
        scopeType === "entity" ? entities : scopeType === "relation" ? relations : [datasetScope];

      items.forEach((item) => {
        const targetId =
          scopeType === "entity" ? item.id : scopeType === "relation" ? item.id : "dataset";
        ...
        if (!guardResult.ok) {
          trace.status =
            trace.guardResult === "validation_error" ? "validation_error" : "guard_blocked";
          if (guardResult.validationErrors?.length) {
            ...
            if (config.diagnostics?.fail_closed !== false) {
              const evidence = emitDiagnosticEvidence(
                rule,
                scopeType,
                targetId,
                guardResult.validationErrors.join(" "),
                state,
              );
          ...
          if (!validation.ok) {
            ...
            if (
              validation.errors.some(
                (error) => error.includes("no runtime handler") || error.includes("not exposed"),
              )
            ) {
        ...
        trace.status = "fired";
        trace.emittedEvidenceIds.push(evidence.id);
        (rule.derive || []).forEach((action) =>
          applyAction(action, context, evidence, state, trace),
        );
        state.ruleTraces.push(trace);
      });
    });
...
export function summarizeLenses(config, lensBuckets, presetId) {
  ...
  if (!scored.length) {
    return [
      { id: "general", label: "General", score: 1, rawScore: 1, role: "primary", evidenceIds: [] },
    ];
  }
...
export function validateConfigWithRegistry(config) {
  ...
  (config.rules || [])
    .filter((rule) => rule.enabled !== false && rule.function)
    .forEach((rule) => {
      const scopeType = inferRuleScope(rule, config);
      const validation = registry.validate(rule.function, rule.args || {}, scopeType);
      if (!validation.ok) {

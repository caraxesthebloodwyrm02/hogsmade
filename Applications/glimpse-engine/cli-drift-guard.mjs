...
`;
  ...
  return { yamlPath: path.join(TEMP_DIR, 'glimpse.master.yaml'), jsPath: path.join(TEMP_DIR, 'default-master.js') };
}
...
async function useCase3_AutoHealing(paths) {
  ...
  const modifiedYaml = readFileSync(paths.yamlPath, 'utf8') + 
  ...
  metric('Drift detected', result.report.drift.detected ? 'YES' : 'NO', result.report.drift.detected ? 'bad' : 'good');
  metric('Severity', result.report.drift.severity, result.report.drift.severity === 'critical' ? 'bad' : 'neutral');
  metric('Action taken', result.decision.action);
  
  if (result.resolution) {
    metric('Resolution status', result.resolution.status, result.resolution.status === 'SUCCESS' ? 'good' : 'bad');
...
async function useCase5_TrendAnalysis(guard) {
  ...
  metric('Drift rate', `${(trends.driftRate * 100).toFixed(1)}%`, trends.driftRate > 0.2 ? 'bad' : 'good');
  metric('Average duration', `${trends.avgDuration.toFixed(0)}ms`);
  metric('Trend direction', trends.trend, trends.trend === 'DEGRADING' ? 'bad' : trends.trend === 'STABLE' ? 'good' : 'neutral');
...
function useCase6_PolicyComparison() {
  ...
  policies.forEach(({ name, policy, env }) => {
    ...
    metric('Auto-heal enabled', policy.autoHeal ? 'YES' : 'NO', policy.autoHeal ? 'good' : 'neutral');

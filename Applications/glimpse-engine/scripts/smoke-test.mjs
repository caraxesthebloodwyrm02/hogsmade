...
async function testModule(name, importPath) {
  ...
  try {
    ...
    console.log(`   Exports: ${Object.keys(module).slice(0, 5).join(', ')}${Object.keys(module).length > 5 ? '...' : ''}`);
    passedTests++;
    return module;
  } catch (err) {
...
if (contracts) {
  testFunction(contracts, 'validateShape', ['Entity', { id: 'test', name: 'Test', type: 'general', dimensions: {}, metrics: {}, evidenceIds: [] }], 'object');
...
const calibration = await testModule('calibration-engine', '../core/validators/calibration-engine.js');
...
const funcContracts = await testModule('function-contract', '../core/validators/function-contract.js');
...
if (funcContracts) {
  ...
  totalTests++;
  try {
    const mockReport = { valid: true, summary: { coverage: 1.0, totalDeclared: 5 }, missing: [], orphaned: [], mismatched: [] };

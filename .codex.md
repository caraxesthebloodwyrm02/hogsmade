...
This is a namespaced multi-project monorepo:

- `Components/shared-types/` — build first (`npm install && npm run build`)
- MCP servers (`Tools/MCPServers/`): afloat, echoes, eligibility, glimpse, grid, lots, maintain, mangrove, ori, overview, pulse, seeds
- `Projects/GRID-main/` — git submodule → GRID-INTELLIGENCE/GRID (Python, `uv sync`)
- `Applications/glimpse-artifact/` — React component library

## MCP Servers

19 MCP servers in `mcp_config.json` (12 TypeScript + 7 Python GRID servers). Python servers use GRID venv at `/home/caraxes/CascadeProjects/Projects/GRID-main/.venv/bin/python`.

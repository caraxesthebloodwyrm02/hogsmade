...
try {
  const lastTag = execSync('git describe --tags --abbrev=0 HEAD~1 2>/dev/null', { encoding: 'utf8' }).trim();
  
  const commits = execSync(`git log ${lastTag}..HEAD --pretty=format:"%s|%h|%an" --no-merges`, { encoding: 'utf8' })
    .split('\n')
    .filter(Boolean);
  ...
  for (const [cat, items] of Object.entries(categories)) {
    ...
    const header = cat === 'feat' ? 'Features' :
                   cat === 'fix' ? 'Bug Fixes' :
                   cat === 'security' ? 'Security' :
                   cat === 'ci' ? 'CI/CD' :
                   cat === 'docs' ? 'Documentation' :
                   cat === 'chore' ? 'Chores' : 'Other';
    console.log(`## ${header}`);

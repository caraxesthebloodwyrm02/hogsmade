...
export function ScenarioCanvasView() {
  ...
  /* ── Actions ───────────────────────────────────────────────────── */
  const addSeed = useCallback((template: ScenarioSeed) => {
    const seed: ScenarioSeed = { ...template, id: nextId('seed'), createdAt: new Date().toISOString() };
    const offsetX = nodes.length * 320 + 80;
    setSeeds((s) => [...s, seed]);
    setNodes((n) => [...n, { type: 'seed', id: seed.id, x: offsetX, y: 80 }]);
    setShowShelf(false);
  }, [nodes.length]);

  const forkFromSeed = useCallback((seedId: string) => {
    const seedNode = nodes.find((n) => n.id === seedId);
    if (!seedNode) return;

    const branchId = nextId('branch');
    const snapshotId = nextId('glimpse');
    const branch: Branch = { id: branchId, seedId, label: `Branch ${branches.length + 1}` };

    const branchCount = branches.filter((b) => b.seedId === seedId).length;
    ...
    setNodes((n) => [...n, { type: 'glimpse', id: snapshotId, x: snapX, y: snapY }]);
    setEdges((e) => [
      ...e,
      {
        from: seedId,
  ...
  const updateAnnotation = useCallback((id: string, text: string) => {
    setAnnotations((a) => a.map((n) => (n.id === id ? { ...n, text } : n)));
  }, []);
  ...
  const toolbarActions = [
    {
      ...
      icon: (
        <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
          ...
          <path d="M10 2v4M10 14v4M2 10h4M14 10h4" stroke="currentColor" strokeWidth="1.5" fill="none" />
        </svg>
      ),
    ...
    <div className="h-full min-h-0 flex flex-col bg-canvas-bg font-body relative">
      ...
      >
        <ScenarioCanvas className="w-full h-full">
          ...
          {/* Glimpse snapshots */}
          {nodes
            .filter((n) => n.type === 'glimpse')
            .map((node) => {
              const snap = snapshots.find((s) => s.id === node.id);
              if (!snap) return null;
              return (
                <GlimpseSnapshotCard
                  key={snap.id}
        ...
        {seeds.length === 0 && !showShelf && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center pointer-events-auto max-w-sm">
              ...
              <p className="font-body text-sm text-ink-muted mb-4">
                Start by choosing a seed — a starting situation for your scenario.
                Then fork it to explore different paths and compare glimpses side by side.
              </p>
              <button
          ...
          <div className="max-w-5xl mx-auto bg-canvas-surface/95 backdrop-blur-sm border border-border-color rounded-lg shadow-token-md p-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              {Array.from(selectedGlimpses).map((gId) => {
                const snap = snapshots.find((s) => s.id === gId);
                if (!snap) return null;
                return (
                  <div
                    key={snap.id}

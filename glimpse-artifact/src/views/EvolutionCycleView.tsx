...
import { ArrowRightLeft, Gauge, GitBranch, Orbit, ShieldAlert, TableProperties, TimerReset } from 'lucide-react';
    ...
    <article className="glass-panel p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {beatRail.map((entry) => (
          ...
          >
            <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-ink-muted">{entry.state}</p>
...
export function MomentumPanel({ momentum }: { momentum: MomentumFrame }) {
  const metrics = [
    ...
    { label: 'Integration success', value: momentum.integrationSuccessRate, accent: 'text-lime-500' },
  ];
  ...
  return (
    <article className="glass-panel p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-3">
        {metrics.map((metric) => (
          <div key={metric.label} className="rounded-xl border border-border-color bg-surface-raised p-4">
            <p className="font-body text-xs uppercase tracking-[0.08em] text-ink-muted">{metric.label}</p>
            <p className={cn('mt-3 font-mono text-2xl', metric.accent)}>{formatDecimal(metric.value)}</p>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm text-ink-muted">
        <div>Reversal rate: <span className="font-mono text-ink">{formatPct(momentum.reversalRate)}</span></div>
        <div>Stale ratio: <span className="font-mono text-ink">{formatPct(momentum.staleWindowRatio)}</span></div>
        <div>Priority conditions: <span className="font-mono text-ink">{momentum.openPriorityConditionCount}</span></div>
      </div>
    </article>
  );
}
    ...
    <article className="glass-panel p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <div className="rounded-xl border border-border-color bg-surface-raised p-4">
          ...
          <p className="mt-2 font-mono text-xl text-ink">{formatDecimal(gate.metrics.overallScore)}</p>
        </div>
        <div className="rounded-xl border border-border-color bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-[0.08em] text-ink-muted">Governance</p>
          <p className="mt-2 font-mono text-xl text-ink">{formatDecimal(gate.metrics.governanceScore)}</p>
        </div>
        <div className="rounded-xl border border-border-color bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-[0.08em] text-ink-muted">Integration</p>
          <p className="mt-2 font-mono text-xl text-ink">{formatDecimal(gate.metrics.integrationScore)}</p>
        </div>
        <div className="rounded-xl border border-border-color bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-[0.08em] text-ink-muted">Drift</p>
          <p className="mt-2 font-mono text-xl text-ink">{formatDecimal(gate.metrics.sidewalkDrift)}</p>
        </div>
      </div>
      <div className="space-y-2">
        {gate.reasons.length > 0 ? gate.reasons.map((reason) => (
          <p key={reason} className="text-sm text-ink-muted">{reason}</p>
        )) : (
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-border-color">
                <th className="py-2 pr-4 text-left text-[10px] uppercase tracking-[0.08em] text-ink-muted">Endpoint</th>
                <th className="py-2 pr-4 text-left text-[10px] uppercase tracking-[0.08em] text-ink-muted">Owner</th>
                <th className="py-2 pr-4 text-left text-[10px] uppercase tracking-[0.08em] text-ink-muted">Contract</th>
                <th className="py-2 pr-4 text-left text-[10px] uppercase tracking-[0.08em] text-ink-muted">Status</th>
                <th className="py-2 pr-4 text-left text-[10px] uppercase tracking-[0.08em] text-ink-muted">Readiness</th>
              </tr>
            </thead>
            <tbody>
              {endpointSpecs.map((spec) => (
                <tr key={spec.id} className="border-b border-border-color/40">
                  ...
                  <td className="py-3 pr-4 font-mono text-ink">{spec.readiness !== undefined ? formatPct(spec.readiness) : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {snapshot.caseRecord.handoffs.length === 0 ? (
        ...
        <div className="space-y-3">
          {snapshot.caseRecord.handoffs.map((handoff) => (
            <div key={handoff.id} className="rounded-xl border border-border-color bg-surface-raised p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="font-body text-sm text-ink">{handoff.from} → {handoff.to}</p>
                <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-violet-500">{handoff.status}</span>
              </div>
              <p className="mt-2 text-sm text-ink-muted">{handoff.summary}</p>
              <p className="mt-2 font-mono text-[11px] text-ink-muted">{formatTime(handoff.recordedAt)}</p>
            </div>
          ))}
        </div>
      )}
...
function NotesPanel({ title, notes }: { title: string; notes: Array<ConditionNote | { id: string; message: string }> }) {
  return (
    <article className="glass-panel p-5 space-y-4">
      ...
      {notes.length === 0 ? (
        ...
        <div className="space-y-3">
          {notes.map((note) => (
            <div key={note.id} className="rounded-xl border border-border-color bg-surface-raised p-4">
              {'severity' in note ? (
                <p className="font-mono text-[11px] uppercase tracking-[0.08em] text-amber-500">{note.severity}</p>
              ) : null}
            ...
            <thead>
              <tr className="border-b border-border-color">
                {table.columns.slice(0, 6).map((column) => (
                  <th key={column} className="py-2 pr-4 text-left text-[10px] uppercase tracking-[0.08em] text-ink-muted">
                    {column}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {table.rows.slice(0, 6).map((row) => (
                <tr key={row.rowId} className="border-b border-border-color/40">
                  ...
                  <td className="py-3 pr-4 font-mono text-[11px] text-lime-600">{row.creditLabel}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function TimelinePanel({ snapshot }: { snapshot: CycleSnapshot }) {
  return (
    <article className="glass-panel p-5 space-y-4">
      ...
      <div className="space-y-3">
        {snapshot.caseRecord.timeline.slice().reverse().map((entry) => (
          <div key={entry.id} className="rounded-xl border border-border-color bg-surface-raised p-4">
            <div className="flex items-center justify-between gap-3">
              <p className="font-body text-sm text-ink">{entry.summary}</p>
              <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-amber-500">{entry.event}</span>
            </div>
            <p className="mt-2 font-mono text-[11px] text-ink-muted">{formatTime(entry.timestamp)}</p>
          </div>
        ))}
      </div>
    </article>
  ...
  return (
    <section className="min-h-full bg-canvas-bg text-ink px-5 py-6 md:px-8 space-y-6">
      <header className="space-y-3 max-w-4xl">
        <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-teal-600">Evolution cycle control room</p>
        ...
        <p className="max-w-3xl text-sm md:text-base text-ink-muted leading-7">
          The control room is UI-first, but the server snapshot is the authority. Beats, momentum, drift, gate status,
          collection rows, and timeline events are rendered directly from the evolution case runtime.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)] gap-5">
        <aside className="space-y-5">
            ...
            <div className="space-y-3">
              {cases.map((item) => (
                ...
                >
                  ...
                  <p className="mt-1 font-mono text-[11px] text-ink-muted">{item.currentBeat} · {item.status}</p>
                  <p className="mt-2 text-xs text-ink-muted">momentum {formatDecimal(item.momentum)} · drift {formatDecimal(item.sidewalkDrift)}</p>
                </button>
              ))}
            </div>
          </article>
          ...
          {selectedCaseId ? (
            <article className="glass-panel p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => void advanceCase(selectedCaseId)} className="rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink">
                  Advance
                </button>
                <button onClick={() => void advanceCase(selectedCaseId, 'return', 'Manual control-room return')} className="rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink">
                  Return
                </button>
                <button onClick={() => void recordSignal(selectedCaseId, 'integration_call_succeeded')} className="rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink">
                  Call success
                </button>
                <button onClick={() => void recordSignal(selectedCaseId, 'integration_call_failed')} className="rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink">
                  Call failure
                </button>
                <button onClick={() => void recordSignal(selectedCaseId, 'heartbeat_stale')} className="rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink">
                  Heartbeat stale
                </button>
                <button onClick={() => void evaluatePromotion(selectedCaseId)} className="rounded-xl border border-amber-400/30 bg-amber-400/10 px-3 py-2 text-sm text-amber-600">
                  Evaluate promotion
                </button>
              </div>
            </article>
          ...
          {selectedCaseId ? (
            <article className="glass-panel p-5 space-y-4">
              ...
              <div className="space-y-3">
                <input value={endpointId} onChange={(event) => setEndpointId(event.target.value)} className="w-full rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink" />
                <input value={endpointLabel} onChange={(event) => setEndpointLabel(event.target.value)} className="w-full rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink" />
                <input value={endpointOwner} onChange={(event) => setEndpointOwner(event.target.value)} className="w-full rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink" />
                <input value={endpointContract} onChange={(event) => setEndpointContract(event.target.value)} className="w-full rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink" />
                <select value={endpointStatus} onChange={(event) => setEndpointStatus(event.target.value as EndpointSpec['status'])} className="w-full rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink">
                  <option value="draft">draft</option>
                  <option value="ready">ready</option>
                  <option value="blocked">blocked</option>
                  <option value="verified">verified</option>
                </select>
                <input value={readiness} onChange={(event) => setReadiness(event.target.value)} className="w-full rounded-xl border border-border-color bg-surface-raised px-3 py-2 text-sm text-ink" />
                <button
                  onClick={() => void upsertEndpoint(selectedCaseId, {
                    endpointId,
                    label: endpointLabel,
                    owner: endpointOwner,
          ...
          {selectedCaseId ? (
            <article className="glass-panel p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <button onClick={() => void recordHandoff(selectedCaseId, {
                  from: 'mapper',
                  to: 'operator',
                  status: 'submitted',
        ...
        <div className="space-y-5">
          {error ? <DataError message={error} onRetry={() => void selectCase(selectedCaseId ?? snapshot?.caseRecord.caseId ?? '')} /> : null}
          {loading && !snapshot ? (
          ...
          {!snapshot ? (
            ...
            <>
              <article className="glass-panel p-5 space-y-3">
                <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-teal-600">Runtime-backed summary</p>
                ...
                {selectedSummary ? (
                  <p className="font-mono text-[11px] text-ink-muted">
                    case {selectedSummary.caseId} · beat {selectedSummary.currentBeat} · updated {formatTime(selectedSummary.updatedAt)}
                  </p>
                ) : null}
              </article>

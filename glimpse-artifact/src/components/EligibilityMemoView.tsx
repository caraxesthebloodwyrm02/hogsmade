...
function ScoreBar({ score, label, rank, band }: { score: number; label: string; rank: number; band?: string }) {
  const bandColor = band ? BAND_COLORS[band] ?? 'var(--sediment-mid)' : 'var(--teal-500)';
  const width = Math.max(2, score * 100);

  return (
    <div className="flex items-center gap-3 group">
      <span className="font-mono text-[10px] text-ink-ghost w-4 text-right shrink-0">
        {rank}
      </span>
...
function MomentumGauge({ value, label, color, thresholdValue }: {
  value: number; label: string; color: string; thresholdValue?: number;
}) {
  const R = 32;
    ...
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: 80, height: 80 }}>
        <svg width="80" height="80" viewBox="0 0 80 80" className="absolute inset-0 -rotate-90">
          <circle cx="40" cy="40" r={R} fill="none" stroke="var(--sediment-dark)" strokeWidth="4" opacity="0.4" />
          <circle
            cx="40" cy="40" r={R}
          ...
          {thresholdValue !== undefined && (
            <circle
              cx="40" cy="40" r={R}
              fill="none" stroke="var(--rose-500)" strokeWidth="1" strokeDasharray="2 4"
  ...
  return (
    <div className="flex items-end gap-1 h-16">
      {PASS_LABELS.map((label, i) => {
        ...
        return (
          <div key={label} className="flex-1 flex flex-col items-center gap-1" title={`${label}: ${count}`}>
            <div
              className="w-full rounded-t-sm transition-all"
              style={{
                height: `${height}%`,
                background: `rgba(78, 205, 196, ${brightness * 0.7})`,
                boxShadow: brightness > 0.6 ? `0 0 6px rgba(78, 205, 196, ${brightness * 0.3})` : 'none',
              }}
            />
...
function BandDistribution({ rows }: { rows: CollectionRow[] }) {
  ...
  const total = Math.max(1, Object.values(bands).reduce((a, b) => a + b, 0));

  return (
    <div className="space-y-1.5">
      {/* Strip */}
      <div className="flex h-3 rounded-full overflow-hidden" style={{ background: 'var(--sediment-dark)' }}>
...
export function EligibilityMemoView({ snapshot, className }: EligibilityMemoViewProps) {
  ...
  for (const row of rows) {
    if (row.rowType === 'dimension' && row.candidateId === topCandidateId && row.dimensionScore !== null) {
  ...
  const overallRows = rows
    .filter(r => r.rowType === 'dimension' && r.dimension === 'overall' && r.dimensionScore !== null)
        ...
        <>
          ...
          <div className="glass-panel px-5 py-4">
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
              ...
              ] as [string, number, number][]).map(([label, threshold, actual]) => {
                const passed = label === 'Drift' ? actual < threshold : actual >= threshold;
                return (
                  <div key={label} className="flex items-center justify-between">
                    <span className="font-mono text-[10px] text-ink-muted uppercase tracking-wider">{label}</span>
